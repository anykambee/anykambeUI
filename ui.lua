...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if Settings.LoadingTitle ~= "Rayfield Interface Suite" then
		LoadingFrame.Version.Text = "Kambe Helper"
	end
	...
	if not Settings.RandomMessage.disable then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				RayfieldLibrary:Notify({
					Title = Settings.RandomMessage.title,
					Content = Settings.RandomMessage.prompt,
					Duration = 7,
					Image = 4370033185,
				})
			end
